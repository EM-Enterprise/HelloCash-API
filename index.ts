...
export async function getArticles(limit: number = -1) {
...
function parseInvoice(invoice: RawInvoices['invoices'][number]): Invoice {
  ...
  return {
    ...
    items: invoice.items?.map((i): Item => ({
...
export async function getCustomers(limit: number = -1) {
  ...
  do {
    ...
    customers.push(...users.map((u): Customer => ({
      id: u.user_id,
...
export async function findExactCustomer(customer: Partial<Customer>){
  ...
  const exactMatches = customers.filter(c => Object.keys(customer).map(_key => {
...
export async function findCustomer(user: Partial<Customer>) {
  ...
  const exactMatches = customers.filter(customer => Object.keys(user).map(_key => {
    const key = _key as keyof Customer
    return customer[key] === user[key]
  }).every((el) => el))

  const partialAddressMatches = customers.filter(customer => customer.country?.trim() === user.country?.trim() && customer.city?.trim() === user.city?.trim() && customer.postCode?.trim() === user.postCode?.trim() && customer.street?.trim() === user.street?.trim() && customer.houseNumber?.trim() === user.houseNumber?.trim())
...
export async function getCategories() {
  ...
  return categories.map((cat): ArticleCategory => ({
...
export async function getStockChanges(id: Article['id']) {
  ...
  return changes.map((change): StockChange => ({
